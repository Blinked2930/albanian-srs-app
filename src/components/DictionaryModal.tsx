...
export default function DictionaryModal({ word, onClose, onUpdate }: { word: any | null; onClose: () => void; onUpdate?: (updatedWord: any) => void }) {
    ...
    useEffect(() => {
        if (!word) { setIsEditing(false); return; }
                    ...
                    <div className="mb-6 relative">
                        {isEditing ? (
                            ...
                            <div className="pr-16 animate-in fade-in">
                                <span className="text-xs font-black uppercase tracking-widest text-indigo-500 mb-2 block bg-indigo-50 inline-block px-3 py-1 rounded-full">{word.type || "Uncategorized"}</span>
